...
// ─── High-zoom tile overlay ──────────────────────────────────────────────
// When the main bitmap is capped (rendering at less than the requested
// scale), this draws a CRISP overlay of the currently-visible viewport
// at the full requested scale. The overlay is sized to the visible
// viewport and positioned absolutely within #canvas-container.

let _tileRenderGen = 0;

async function _renderTileOverlay(doc, pageNum, scale, viewport) {
  ...
  const myGen = ++_tileRenderGen;

  // Compute the visible region of the page in canvas-container CSS coords.
  const containerRect = container.getBoundingClientRect();
  const canvasContainerRect = canvasContainer.getBoundingClientRect();

  const visLeft_css = Math.max(0, containerRect.left - canvasContainerRect.left);
  const visTop_css = Math.max(0, containerRect.top - canvasContainerRect.top);
  const visRight_css = Math.min(viewport.width, visLeft_css + container.clientWidth);
  const visBottom_css = Math.min(viewport.height, visTop_css + container.clientHeight);
  const visW_css = Math.max(0, visRight_css - visLeft_css);
  const visH_css = Math.max(0, visBottom_css - visTop_css);

  if (visW_css < 10 || visH_css < 10) {
    tileCanvas.style.display = 'none';
    return;
  }

  // Convert CSS coords to PDF points (1 css pixel = 1/scale point at this zoom).
  const regionX_pt = visLeft_css / scale;
  const regionY_pt = visTop_css / scale;
  const regionW_pt = visW_css / scale;
  const regionH_pt = visH_css / scale;

  try {
    const { invoke: _tileInvoke } = window.__TAURI__.core;
    const t0 = performance.now();
    const rgbaData = await _tileInvoke('render_pdf_page_region', {
      path: doc.filePath,
      pageIndex: pageNum - 1,
      scale: scale,
      regionXPt: regionX_pt,
      regionYPt: regionY_pt,
      regionWPt: regionW_pt,
      regionHPt: regionH_pt,
    });
    if (myGen !== _tileRenderGen) {
      console.log(`[tile] STALE gen ${myGen} (current ${_tileRenderGen}) — skipping`);
      return;
    }
    const bytes = rgbaData instanceof Uint8Array ? rgbaData : new Uint8Array(rgbaData);
    if (!bytes || bytes.length <= 8) {
      console.warn('[tile] empty response');
      return;
    }
    ...
    if (w * h * 4 !== bytes.length - 8) {
      console.warn(`[tile] size mismatch ${w}x${h}x4 != ${bytes.length - 8}`);
      return;
    }
    ...
    tileCanvas.style.top = Math.floor(visTop_css) + 'px';
    tileCanvas.style.display = 'block';
    tileCanvas.getContext('2d').putImageData(new ImageData(rgba, w, h), 0, 0);

    const ms = Math.round(performance.now() - t0);
    console.log(`[tile] rendered ${w}x${h} at scale=${scale.toFixed(2)} in ${ms}ms`);
  } catch (e) {
    console.warn('[tile] render failed:', e);
  }
}

function _hideTileOverlay() {
  const tileCanvas = document.getElementById('pdf-canvas-tile');
  if (tileCanvas) tileCanvas.style.display = 'none';
  _tileRenderGen++;
}

// Scroll listener: re-renders tile overlay on scroll when cap is active.
let _tileScrollDebounce = null;
function _scheduleTileRerenderOnScroll() {
  if (_tileScrollDebounce) clearTimeout(_tileScrollDebounce);
  _tileScrollDebounce = setTimeout(() => {
    _tileScrollDebounce = null;
    const doc = state.documents[state.activeDocumentIndex];
    if (!doc?.pdfDoc) return;
    const tileCanvas = document.getElementById('pdf-canvas-tile');
    if (!tileCanvas || tileCanvas.style.display === 'none') return;
    doc.pdfDoc.getPage(doc.currentPage).then(page => {
      const extraRot = getPageRotation(doc.currentPage);
      const opts = { scale: doc.scale };
      if (extraRot) opts.rotation = (page.rotate + extraRot) % 360;
      const vp = page.getViewport(opts);
      _renderTileOverlay(doc, doc.currentPage, doc.scale, vp).catch(() => {});
    }).catch(() => {});
  }, 150);
}

// Wire scroll listener to #pdf-container once (called from initSinglePage
// below, safe to call multiple times — idempotent via the _tileScrollWired flag).
let _tileScrollWired = false;
export function wireTileScrollListener() {
  if (_tileScrollWired) return;
  const container = document.getElementById('pdf-container');
  if (!container) return;
  container.addEventListener('scroll', _scheduleTileRerenderOnScroll, { passive: true });
  _tileScrollWired = true;
}
